...
An example of how your file should look like is present within the `Datasets` folder (`vegetation_plots_example.csv`). After the inference, your file will be overwritten with a new `Habitat` column (if habitat prediction is enabled) and/or new `Likely missing species`, `Scores of likely missing species`, `Positions of likely missing species`, and `Completed vegetation plots` columns (if species prediction is enabled).
...
- before curating a dataset, make sure it contains enough vegetation data (the more the better, for the number of vegetation plots, plant observations, and unique species).
...
- **Leblanc, C.**, et al. (2025). Learning the syntax of plant assemblages. In *Nature Plants*. ([Pl@ntBERT used to predict habitat types and missing plant species](https://doi.org/10.1038/s41477-025-02105-7))
- **Leblanc, C.**, et al. (2025). Mapping biodiversity at very-high resolution in Europe. In *Proceedings of the Computer Vision and Pattern Recognition Conference*. ([Pl@ntBERT used to map the distribution of habitats in Europe](https://doi.org/10.1109/CVPRW67362.2025.00221))
- **Picek, L.**, et al. (2025). GeoPl@ntNet: A Platform for Exploring Essential Biodiversity Variables. In *Proceedings of the European Conference on Artificial Intelligence*. ([Pl@ntBERT used in a platform for exploring habitats distribution predictions](https://doi.org/10.3233/FAIA251430))
- **Palard, R.**, et al. (2025). GeoPl@ntNet: A High-Resolution Biodiversity Mapping Platform for Environmental Forensics. In *Biodiversity Information Science and Standards*. ([Pl@ntBERT used to predict standardized habitat types for environmental forensics](https://doi.org/10.3897/biss.9.181374))
- **Henriet, C.**, et al. (2026). Refining the pollen signal of mountain peatlands: A phytosociology-based calibration in the Massif central (France). In *Review of Palaeobotany and Palynology*. ([Pl@ntBERT used to classify surveys into habitats without using abundance](https://doi.org/10.1016/j.revpalbo.2026.105687))
